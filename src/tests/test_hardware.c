...
static void TestSumRecursiveCondition()
{
    ...
    for (int i = 0; i < 19; ++ i)
    {
        ...
        //为什么是0x00400000
        //估计是随机取了个地址
        writeinst_dram(va2pa(i * MAX_INSTRUCTION_CHAR + 0x00400000, cr), assembly[i], cr);
    }
    //由于每条指令都是定长MAX_INSTRUCTION_CHAR=64个字符
    //16是上面的第16条指令
    //0x00400000地址空间的开始地址
    //取到的是实际的物理地址的编号，赋给了cr->rip
    cr->rip = MAX_INSTRUCTION_CHAR * sizeof(char) *16 + 0x00400000;

    printf("begin\n");
    int time = 0;

    while ((cr->rip <= 18  * MAX_INSTRUCTION_CHAR + 0x00400000) &&
